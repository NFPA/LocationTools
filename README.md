# Location Tools (a.k.a. Wandering Moose): Geocoder for huge datasets 

The motivation to create this project comes from the fact that geocoding millions of address points is painful, time consuming and often very expensive process. One such huge dataset is NFIRS ( [National Fire Incident Reporting System](https://www.usfa.fema.gov/data/nfirs/index.html) ) which collects and maintain fire incidents from various fire departments across the United States. This dataset is available to everyone upon request and it contains many address related fields that can be used for spatial analytics use cases. Although, readily available dataset the caveat to the problem is the volume and varying degree of quality of information available. NFIRS collects about 20-25 million addresses (fire incidents) every year with some of the more detailed address fields being optional to fill in for simplicity.

This project, codenamed 'Wandering Moose' was created to solve some aspects of making use of such huge datasets for downstream spatial analysis. Specifically focusing on speed and cost by compromising on some of the accuracy of the results. This assumption was made as not all spatial use cases necessarily require roof top accuracy, therefore the result’s accuracy vary by the amount of information you provide to the system. 

This tool was initially geared towards NFIRS type dataset, we presume that it can be extended to any dataset with address fields. We provide you with sufficient documentation related to how it can be setup in different ways (Server or Docker) with code, examples, and sample result outputs. See Documentaion Section.
